...
function generateInvoicePDF(invoiceNum, clientName, clientEmail, plan, amount, date) {
  return new Promise((resolve, reject) => {
    ...
    // ── Header ──
    const logoPath = path.join(__dirname, 'public', 'logo_sindingsocial.png');
    if (fs.existsSync(logoPath)) {
      doc.image(logoPath, 50, 45, { height: 40 });
      doc.fontSize(10).font('Helvetica').fillColor('#888888').text(SINDING_ADDRESS, 50, 90);
    } else {
      doc.fontSize(20).font('Helvetica-Bold').fillColor('#111111').text(SINDING_NAME, 50, 50);
      doc.fontSize(10).font('Helvetica').fillColor('#888888').text(SINDING_ADDRESS, 50, 76);
    }

    doc.fontSize(28).font('Helvetica').fillColor('#333333').text('Invoice', 50, 50, { align: 'right' });
    doc.fontSize(12).font('Helvetica').fillColor('#888888').text(`#${invoiceNum}`, 50, 84, { align: 'right' });

    // ── Separator ──
    doc.moveTo(50, 110).lineTo(545, 110).strokeColor('#111111').lineWidth(1.5).stroke();

    // ── Meta ──
    doc.fillColor('#888888').fontSize(9).font('Helvetica-Bold')
      .text('BILLED TO', 50, 126)
      .text('DATE', 400, 126);

    doc.fillColor('#111111').fontSize(11).font('Helvetica')
      .text(clientName, 50, 142)
      .text(clientEmail, 50, 158)
      .text(dateStr, 400, 142);

    // ── Table header ──
    const tableTop = 210;
    doc.rect(50, tableTop, pageW, 28).fill('#f5f5f3');
    doc.fillColor('#888888').fontSize(9).font('Helvetica-Bold')
      .text('DESCRIPTION', 60, tableTop + 10)
      .text('AMOUNT', 60, tableTop + 10, { align: 'right', width: pageW - 20 });

    // ── Table row ──
    doc.fillColor('#111111').fontSize(11).font('Helvetica')
      .text(`The Strategy Studio — ${planLabel}`, 60, tableTop + 44)
      .text(`€${amount.toFixed(2)}`, 60, tableTop + 44, { align: 'right', width: pageW - 20 });

    doc.moveTo(50, tableTop + 68).lineTo(545, tableTop + 68).strokeColor('#e0e0e0').lineWidth(0.5).stroke();

    // ── Total ──
    doc.moveTo(50, tableTop + 80).lineTo(545, tableTop + 80).strokeColor('#111111').lineWidth(1.5).stroke();
    doc.fillColor('#111111').fontSize(13).font('Helvetica-Bold')
      .text('Total', 60, tableTop + 92)
      .text(`€${amount.toFixed(2)}`, 60, tableTop + 92, { align: 'right', width: pageW - 20 });

    // ── Footer ──
    doc.fillColor('#aaaaaa').fontSize(9).font('Helvetica')
      .text('Thank you for your subscription to The Strategy Studio.', 50, 700, { align: 'center', width: pageW });
...
async function createAndSendInvoice(subscriberId, email, name, plan, amount) {
  try {
    ...
    // Générer PDF
    const pdfBuffer = await generateInvoicePDF(invoiceNum, name, email, plan, amount, new Date());

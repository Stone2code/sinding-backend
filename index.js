...
async function initDB() {
  ...
  await db.query(`ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS phone TEXT;`);
  await db.query(`ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS address TEXT;`);
  ...
  `);
  await db.query(`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS phone TEXT;`);
  await db.query(`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS address TEXT;`);
...
// ─── Infos Sinding Social (entête + bas de page invoice) ─────────────────────
const HELENA_PHONE      = "06.78.63.23.11";
const HELENA_BIZ_EMAIL  = "Helena@sindingsocial.com";
const MONACO_ADDRESS_1  = 'C/° MCBC « Le Metropole »';
const MONACO_ADDRESS_2  = "17 AV. des Spélugues";
const MONACO_ADDRESS_3  = "MC 98000 Monaco";
const RCI_NUMBER        = "RCI Monaco N°26P 10896";
const TVA_NUMBER        = "N°TVA FR39000191972";
const BANK_NAME         = "Banque LCL HELENA SINDING";
const BANK_IBAN         = "IBAN MC76 3000 2032 6000 0007 3393 E24";
const BANK_BIC          = "BIC CRLYMCM1";
const SINDING_WEBSITE   = "www.sindingsocial.com";

// ─── Helper : génère un PDF invoice avec PDFKit ───────────────────────────────
function generateInvoicePDF(invoiceNum, clientName, clientEmail, clientPhone, clientAddress, plan, amount, date) {
  return new Promise((resolve, reject) => {
    ...
    const planLabel = plan === 'yearly' ? 'Yearly Membership' : 'Monthly Membership (basic)';
    const sentDate  = new Date(date);
    const dueDate   = new Date(date); dueDate.setDate(dueDate.getDate() + 7);
    const fmt       = d => d.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: '2-digit' });
    const pageW     = doc.page.width - 100;

    // ── Header : Helena contact info ──
    doc.fontSize(11).font('Helvetica-Bold').fillColor('#111111').text('SINDING Helena', 50, 50);
    doc.fontSize(10).font('Helvetica').fillColor('#111111')
      .text(HELENA_BIZ_EMAIL, 50, 50, { align: 'right' })
      .text(HELENA_PHONE, 50, 64, { align: 'right' });

    doc.moveTo(50, 95).lineTo(545, 95).strokeColor('#111111').lineWidth(1).stroke();

    // ── INVOICE title + number (left) ──
    doc.fontSize(24).font('Helvetica-Bold').fillColor('#c0622f').text('INVOICE', 50, 120);
    doc.fontSize(10).font('Helvetica').fillColor('#111111')
      .text('Invoice number:', 50, 160)
      .text(invoiceNum, 50, 174)
      .text('Monaco', 50, 188)
      .text('Invoice sent:', 50, 202)
      .text(fmt(sentDate), 50, 216)
      .text('invoice due:', 50, 230)
      .text(fmt(dueDate), 50, 244);

    // ── Client info (right) ──
    doc.fontSize(11).font('Helvetica').fillColor('#111111')
      .text(clientName, 300, 120, { align: 'right', width: 245 });
    let cy = 136;
    if (clientAddress) {
      doc.fontSize(10).text(clientAddress, 300, cy, { align: 'right', width: 245 });
      cy += 16 * Math.ceil(clientAddress.length / 40);
    }
    doc.fontSize(10).text(clientEmail, 300, cy, { align: 'right', width: 245 });
    cy += 16;
    if (clientPhone) {
      doc.text(clientPhone, 300, cy, { align: 'right', width: 245 });
    }

    // ── Table ──
    const tableTop = 300;
    doc.rect(50, tableTop, pageW, 28).fillAndStroke('#faf6e8', '#111111');
    doc.fillColor('#111111').fontSize(9).font('Helvetica-Bold')
      .text('DESCRIPTION', 60, tableTop + 10)
      .text('PRICE', 320, tableTop + 10, { width: 90, align: 'center' })
      .text('TOTAL', 410, tableTop + 10, { width: 90, align: 'center' });

    const rowH = 40;
    doc.rect(50, tableTop + 28, pageW, rowH).strokeColor('#111111').lineWidth(1).stroke();
    doc.fontSize(10).font('Helvetica').fillColor('#111111')
      .text(`The Strategy Studio ${planLabel}`, 60, tableTop + 28 + 14, { width: 250 })
      .text(`${amount.toFixed(0)} €`, 320, tableTop + 28 + 14, { width: 90, align: 'center' })
      .text(`${amount.toFixed(0)} €`, 410, tableTop + 28 + 14, { width: 90, align: 'center' });

    const vatRowH = 70;
    const vatTop  = tableTop + 28 + rowH;
    doc.rect(50, vatTop, pageW, vatRowH).strokeColor('#111111').lineWidth(1).stroke();
    doc.fontSize(8).font('Helvetica').fillColor('#111111')
      .text('VAT: 0%\nSUPPLY OF SERVICES TO A TAXABLE\nPERSON OUTSIDE THE EU –\nVAT NOT APPLICABLE', 320, vatTop + 10, { width: 90, align: 'center' })
      .fontSize(10).text('0 €', 410, vatTop + 28, { width: 90, align: 'center' });

    const totalTop = vatTop + vatRowH;
    doc.rect(50, totalTop, pageW, 30).strokeColor('#111111').lineWidth(1).stroke();
    doc.fontSize(10).font('Helvetica-Bold')
      .text('TOTAL', 320, totalTop + 10, { width: 90, align: 'center' })
      .text(`${amount.toFixed(0)} € EUR`, 410, totalTop + 10, { width: 90, align: 'center' });

    // ── Footer : Monaco address / RCI / TVA ──
    const footTop = 680;
    doc.fontSize(9).font('Helvetica').fillColor('#111111')
      .text(MONACO_ADDRESS_1, 50, footTop)
      .text(MONACO_ADDRESS_2, 50, footTop + 13)
      .text(MONACO_ADDRESS_3, 50, footTop + 26)
      .text(RCI_NUMBER, 300, footTop, { align: 'right', width: 245 })
      .text(TVA_NUMBER, 300, footTop + 13, { align: 'right', width: 245 });

    doc.fontSize(9).font('Helvetica').fillColor('#111111')
      .text(BANK_NAME, 50, footTop + 55, { align: 'center', width: pageW })
      .text(BANK_IBAN, 50, footTop + 68, { align: 'center', width: pageW })
      .text(BANK_BIC, 50, footTop + 81, { align: 'center', width: pageW });

    doc.fontSize(9).fillColor('#888888')
      .text(SINDING_WEBSITE, 50, footTop + 110, { align: 'center', width: pageW });
...
async function createAndSendInvoice(subscriberId, email, name, phone, address, plan, amount) {
  try {
    ...
    // Sauvegarder en DB
    await db.query(`
      INSERT INTO invoices (invoice_num, subscriber_id, email, name, phone, address, plan, amount)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
    `, [invoiceNum, subscriberId, email, name, phone, address, plan, amount]);

    // Générer PDF
    const pdfBuffer = await generateInvoicePDF(invoiceNum, name, email, phone, address, plan, amount, new Date());

    // Envoyer par email à Helena avec PDF en pièce jointe
...
async function chargeSubscriber(subscriber) {
  ...
  try {
    ...
    await createAndSendInvoice(subscriber.id, subscriber.email, subscriber.name, subscriber.phone, subscriber.address, subscriber.plan, amount);
...
function checkoutScript(endpoint) {
  return `<script>
    document.getElementById('form').addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch('${endpoint}', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({name, email, phone, address}) });
        const data = await res.json();
...
const formHTML = `
  <form id="form">
    ...
    <input type="tel" id="phone" placeholder="+33 6 12 34 56 78" required autocomplete="tel">
    <label for="address">Address</label>
    <input type="text" id="address" placeholder="199 Rue des Lilas, 06000 Nice" required autocomplete="street-address">
...
app.post("/create-payment", async (req, res) => {
  ...
  try {
    ...
    pendingOrders[orderRef] = { name, email, phone, address, plan: 'monthly' };
    const token = await getCarloToken();
...
app.post("/create-payment/yearly", async (req, res) => {
  ...
  try {
    ...
    pendingOrders[orderRef] = { name, email, phone, address, plan: 'yearly' };
    const token = await getCarloToken();
    const response = await fetch(`${CARLO_API}/transactions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
  ...
  if (status === "COMPLETED") {
    ...
    try {
      const result = await db.query(`
        INSERT INTO subscribers (email, name, phone, address, card_id, plan, status, retry_count, next_billing_at)
      ...
      `, [email, name, phone, address, cardId?.toString(), plan, nextBilling]);

      const subscriberId = result.rows[0].id;
      const savedPhone   = result.rows[0].phone;
      const savedAddress = result.rows[0].address;
      console.log(`💾 Subscriber ${plan} sauvegardé — prochain débit: ${nextBilling.toISOString()}`);

      // Générer et envoyer invoice
      await createAndSendInvoice(subscriberId, email, name, savedPhone, savedAddress, plan, amount);
